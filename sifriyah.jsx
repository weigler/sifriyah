...
function migrarDados(parsed) {
  ...
  return {
    ...
    config: {
      ...
      whatsappContato: "",
      linkVitrine: "",
      promocao: { ativa: false, descricao: "", validoAte: "" },
      ...(parsed.config || {}),
    },
  };
}
...
export default function App() {
  ...
  async function apagarTudoEComecarDeNovo() {
    ...
    setPessoas([]);
    setCobrancas([]);
    setConfig({ pix: "", recebedor: "", whatsappContato: "", linkVitrine: "", promocao: { ativa: false, descricao: "", validoAte: "" } });
    setSenhaAtual("");
    setUnlocked(false);
    setTemDadosSalvos(false);
  }
      ...
      try {
        ...
        if (cloudConfig) {
          ...
          const dadosPublicos = {
            ...
            livros: livros.map((l) => {
              const ativo = emprestimosAtivos.find((e) => e.livroId === l.id);
              return {
                id: l.id,
                titulo: l.titulo,
                autor: l.autor || "",
                ...
                disponivel: !ativo,
                proximaData: ativo ? ativo.prazo : null,
              };
            }),
          };
  ...
  }, [livros, emprestimos, pessoas, cobrancas, config, loaded, unlocked, senhaAtual, cloudConfig, cloudDocId]);
    ...
    <Section eyebrow="Catálogo" title="Acervo">
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          <Input placeholder="Categoria (ex: teologia)" value={categoria} onChange={(e) => setCategoria(e.target.value)} style={{ flex: "1 1 140px" }} />
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <Input
            type="number"
        ...
        <Button
          ...
          onClick={() => {
            onAdd({ titulo, autor, paginas, dataAquisicao, capaUrl, valorSemanal, limiteSemanas, categoria });
            setTitulo("");
            setAutor("");
            setPaginas("");
            setDataAquisicao("");
            ...
            >
              {editando ? (
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    <Input placeholder="Categoria" value={editCategoria} onChange={(e) => setEditCategoria(e.target.value)} style={{ flex: "1 1 130px" }} />
...
function AjustesTab({ config, onChange, cloudConfig, cloudStatus, onConfigurarNuvem, onDesligarNuvem }) {
  ...
  function salvarGeral() {
    onChange({
      ...
      whatsappContato,
      linkVitrine,
      promocao: { ativa: promoAtiva, descricao: promoDescricao, validoAte: promoValidoAte },
    });
  }
  ...
  return (
    <Section eyebrow="Cobrança" title="Ajustes">
      ...
      >
        ...
        <label style={labelStyle}>Seu WhatsApp (usado na vitrine pública, pra reservas)</label>
      ...
      {cloudConfig && (
        ...
        >
          ...
          <div style={{ fontSize: 12.5, color: COLORS.inkSoft }}>
            Código da biblioteca pra colar no arquivo <code>catalogo.html</code>: <b>{cloudConfig.docId}</b>
          </div>
          ...
          <Input
            ...
            placeholder="https://weigler.github.io/sifriyah/catalogo.html"
          />
          <Button style={{ alignSelf: "flex-start" }} onClick={salvarGeral}>
            Salvar link
          </Button>
