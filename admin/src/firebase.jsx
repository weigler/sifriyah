...
// se a pessoa desligar a nuvem de propósito (Ajustes > Sincronização > "Desligar
// sincronização"), esse aparelho fica marcado aqui — sem isso, o próximo recarregamento
// cairia de novo na configuração padrão embutida (abaixo) e "religaria" sozinho a nuvem
const CLOUD_DESLIGADA_KEY = "sifriyah-cloud-desligada";
function nuvemDesligadaLocal() {
  return localStorage.getItem(CLOUD_DESLIGADA_KEY) === "1";
}
function marcarNuvemDesligadaLocal(desligada) {
  if (desligada) localStorage.setItem(CLOUD_DESLIGADA_KEY, "1");
  else localStorage.removeItem(CLOUD_DESLIGADA_KEY);
}
...
function lerConfigNuvem() {
  try {
    ...
    // ignora e cai na configuração padrão abaixo
  }
  if (nuvemDesligadaLocal()) return null;
  // nenhuma configuração salva ainda neste aparelho: usa a configuração padrão do projeto,
  // já embutida em src/firebase-config.jsx — não precisa mais colar isso na mão em cada
  // aparelho novo. O login de administrador (e-mail/senha) continua sendo exigido do mesmo jeito.
  if (typeof FIREBASE_CONFIG_PADRAO !== "undefined" && FIREBASE_CONFIG_PADRAO && FIREBASE_CONFIG_PADRAO.apiKey) {
    return { ...FIREBASE_CONFIG_PADRAO, docId: (typeof DOC_ID_PADRAO !== "undefined" && DOC_ID_PADRAO) || "principal" };
  }
  return null;
}
function salvarConfigNuvem(cfg) {
  if (cfg) {
    ...
    marcarNuvemDesligadaLocal(false);
  } else {
    localStorage.removeItem(CLOUD_CONFIG_KEY);
    marcarNuvemDesligadaLocal(true);
  }
}
